...
import { useEmCashValue } from '@/hooks/useEmCashValue';
    ...
    const Dashboard = () => {
      ...
      const [data, setData] = useState<{ lancamentos: Lancamento[] }>({ lancamentos: [] });
      const [loading, setLoading] = useState(false);
      const [importLoading, setImportLoading] = useState(false);
      const [chartData, setChartData] = useState<Array<{ month: string; pagar: number; receber: number }>>([]);
      ...
      const loadDataFromSupabase = async () => {
        ...
        if (error) {
          ...
          setData({ lancamentos: (lancamentos ?? []) as Lancamento[] });
        }
        setLoading(false);
      };
    
      const generateChartData = (
        financialData: { lancamentos: Lancamento[] },
        span = monthsSpan,
        cashValue = 0
      ) => {
        const months = [];
