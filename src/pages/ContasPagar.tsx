...
import { Calendar, Filter, Building, DollarSign, AlertTriangle, ArrowLeft, CheckCircle, Settings } from 'lucide-react';
    ...
    type Filters = {
      fornecedor: string;
      status: 'todos' | 'vencido' | 'aberto' | 'pago';
      unidade: string;
      dataInicio: string;
      dataFim: string;
    };
    ...
    const ContasPagar = () => {
      ...
      const loadData = async () => {
        ...
        if (error) {
          ...
          setContas((data ?? []) as Lancamento[]);
        }
        setLoading(false);
      };
    
      const getStatus = (conta: Lancamento) => {
      ...
      const filteredContas = useMemo(() => {
        let filtered: Lancamento[] = [...contas];
      ...
      const groupedContas = useMemo<Record<string, Lancamento[]>>(() => {
        return filteredContas.reduce<Record<string, Lancamento[]>>((acc, conta) => {
          (acc[conta.data] = acc[conta.data] || []).push(conta);
          return acc;
        }, {});
      }, [filteredContas]);
    
      const calculateTotalsByUnit = (itens: Lancamento[]) => {
        return itens.reduce<Record<string, number>>((acc, conta) => {
            ...
            <Card className="glass-card">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  ...
                  <div><label className="text-sm text-gray-300 mb-2 block">Status</label><Select value={filters.status} onValueChange={(value) => setFilters({ ...filters, status: value as Filters['status'] })}><SelectTrigger className="bg-white/10 border-white/20 text-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="todos">Todos</SelectItem><SelectItem value="aberto">Em Aberto</SelectItem><SelectItem value="vencido">Vencido</SelectItem><SelectItem value="pago">Pago</SelectItem></SelectContent></Select></div>
